        ...
        <section hidden={selected !== 0} className={styles.toolPanel} aria-labelledby="cash-plan-title">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
            <div className="grid gap-x-5 gap-y-5 sm:grid-cols-2">
              ...
              ] as const).map(([key, label, note]) => <label key={key} className="block text-sm font-semibold">{label}<div className="mt-2 flex items-center rounded-md border border-navy/20 bg-white focus-within:ring-2 focus-within:ring-gold"><span className="pl-3 text-slateink">{key === "down" || key === "closing" ? "%" : "$"}</span><input type="number" min="0" max={key === "down" || key === "closing" ? 100 : 100_000_000} step={key === "down" || key === "closing" ? 0.1 : 1} value={data.cash[key]} onChange={(event) => { const number = Math.min(key === "down" || key === "closing" ? 100 : 100_000_000, Math.max(0, Number(event.target.value) || 0)); setData((current) => ({ ...current, cash: { ...current.cash, [key]: number } })); setChanged(true); }} className="min-w-0 flex-1 rounded-md bg-transparent px-3 py-3 text-base outline-none" /><span className="sr-only">{note}</span></div><span className="mt-1 block text-xs font-normal leading-relaxed text-slateink">{note}</span></label>)}
            </div>
